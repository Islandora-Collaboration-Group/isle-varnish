...
backend apache { # Define one backend
  .host = "{{getv "/varnish/backend"}}"; # IP or Hostname of backend
  .port = "{{getv "/varnish/backend/port"}}"; # Port of Apache, defined backend or whatever is listening
  .max_connections = {{getv "/varnish/max/connections"}}; # Max 300 for now
...
sub vcl_recv {
  ...
  # Normalize the header, remove the port (in case you are testing this on various TCP ports)
...
/*    if (!std.ip(req.http.X-Forwarded-For, "0.0.0.0") ~ purge) { # purge is the ACL defined at the begining
  ...
  if (req.http.Cookie) {
    ...
    if (req.http.Cookie == "") {
      ...
      # cache the page. Pass it on to Apache or the defined backend directly.
      return (pass);
    }
  }
  ...
  # like msnbot that send no-cache with every request.
    if (! (req.http.Via || req.http.User-Agent ~ "(?i)bot" || req.http.X-Purge)) {
      #set req.hash_always_miss = true; # Does not seems to refresh the object in the cache
...
sub vcl_pass {
  ...
  # backend response is passed on to the client, but is not entered into the cache. Subsequent
...
sub vcl_backend_response {
  ...
  if (bereq.url ~ "^[^?]*\.(7z|avi|bz2|flac|flv|gz|mka|mkv|mov|mp3|mp4|mpeg|mpg|ogg|ogm|opus|rar|tar|tgz|tbz|txz|wav|webm|xz|zip|csv)(\?.*)?$") {
    unset beresp.http.set-cookie;
    set beresp.do_stream = true;  # Check memory usage it will grow in fetch_chunksize blocks (128k by default) if the backend does not send a Content-Length header, so only enable it for big objects
    set beresp.do_gzip   = false;   # Do not try to compress it for storage
  }
  ...
  if (beresp.ttl <= 0s || beresp.http.Set-Cookie || beresp.http.Vary == "*") {
    set beresp.ttl = 120s; # Important, you should not rely on this, SET YOUR HEADERS in the backend
